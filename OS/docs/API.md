...
Programmatic access to the SperaxOS agent marketplace index.
...
```bash
curl https://sperax.works/sperax-ai-agents/index.json
```
...
```bash
curl https://sperax.works/sperax-ai-agents/defi-yield-optimizer.json
```
...
```bash
curl https://sperax.works/sperax-ai-agents/defi-yield-optimizer.zh-CN.json
```
...
function searchAgents(query) {
  return fetch('https://sperax.works/sperax-ai-agents/index.json')
    .then((r) => r.json())
    .then((data) =>
      data.agents.filter(
        (agent) =>
...
async function loadAgent(identifier, locale = 'en-US') {
  ...
  const response = await fetch(`https://sperax.works/sperax-ai-agents/${filename}`);
...
async function safeLoadAgent(identifier) {
  try {
    const response = await fetch(`https://sperax.works/sperax-ai-agents/${identifier}.json`);
...
function searchAgents(query) {
  https.get('https://sperax.works/sperax-ai-agents/index.json', (res) => {
    let data = '';
...
- Issues: github.com/speraxos/sperax-ai-agents/issues
- Email: <api@sperax.io>
